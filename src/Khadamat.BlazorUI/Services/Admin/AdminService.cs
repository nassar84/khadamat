...
public class AdminService : IAdminService
{
    ...
    {
        var response = await _http.PostAsJsonAsync("v1/admin/users", dto);
    ...
    {
        await _http.PostAsync($"v1/admin/services/{id}/approve", null);
    }

    public async Task RejectService(int id)
    {
        await _http.PostAsync($"v1/admin/services/{id}/reject", null);
    }
    ...
    {
        await _http.PostAsync($"v1/admin/providers/{id}/approve", null);
    }

    public async Task RejectProvider(int id)
    {
        await _http.PostAsync($"v1/admin/providers/{id}/reject", null);
    }
    ...
    {
        var response = await _http.PostAsJsonAsync($"v1/admin/users/{id}/role", role);
        response.EnsureSuccessStatusCode();
    }

    public async Task ChangePassword(ChangePasswordDto dto)
    {
        var response = await _http.PostAsJsonAsync("v1/admin/users/change-password", dto);
        response.EnsureSuccessStatusCode();
    }
